      ...
      <div className="relative grid items-center gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
          ...
          <div className="relative overflow-hidden rounded-sm border border-line bg-white shadow-luxe">
            {hasPhoto ? (
              <>
                ...
                <span
                  aria-hidden
                  className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-obsidian-950/70 to-transparent"
                />
              </>
            ) : (
              /* No photo yet — an engraved monogram instead of a stock face */
              <div className="relative flex aspect-[4/5] w-full flex-col items-center justify-center gap-6 bg-paper-100 texture-grain px-8 text-center">
                <span className="font-serif text-[4.5rem] leading-none text-gold-600">{initials || "AM"}</span>
                <span className="h-px w-16 bg-gold-600/50" />
                <span className="font-sans text-[10px] uppercase tracking-[0.28em] text-ink-400">
                  Upload a portrait in the studio
                </span>
              </div>
            )}
            <span aria-hidden className="absolute inset-4 border border-white/50" />
          </div>

          {owner.signatureUrl && (
            <div className="mt-6 border border-line bg-white p-5 shadow-card">
              {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div>
          ...
          <Reveal className="mt-9" delay={80}>
            <p className="font-serif text-[1.9rem] leading-tight text-ink-900 sm:text-[2.2rem]">{owner.name}</p>
            {owner.role && (
              <p className="mt-3 font-sans text-[11px] font-semibold uppercase tracking-[0.24em] text-gold-700">
                {owner.role}
              </p>
            )}
            {owner.photoUrl && (
              <span className="mt-6 block h-px w-24 bg-gradient-to-r from-gold-600/70 to-transparent" />
            )}
          </Reveal>
          ...
          <Reveal className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-5" delay={220}>
            {owner.ctaLabel && owner.ctaHref && (
              <Link href={owner.ctaHref} className="btn-editorial group">
                {owner.ctaLabel}
                <ArrowUpRight className="h-4 w-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
              </Link>
            )}

            <span className="flex flex-wrap items-center gap-x-6 gap-y-3 text-[12.5px] text-ink-500">
              {owner.email && (
                <a href={`mailto:${owner.email}`} className="inline-flex items-center gap-2 transition-colors hover:text-gold-700">
          ...
          {owner.videoUrl && (
            <Reveal className="mt-11" delay={300}>
              <div className="mb-4 flex items-center gap-2.5 font-sans text-[10.5px] uppercase tracking-[0.24em] text-gold-700">
                <Play className="h-3.5 w-3.5" />
                A personal message
              </div>
              <div className="overflow-hidden rounded-sm border border-line bg-white shadow-card">
